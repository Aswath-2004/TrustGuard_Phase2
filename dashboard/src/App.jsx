...
// ─── API BASE URL ─────────────────────────────────────────────
// Uses VITE_API_URL env var (set in Vercel dashboard), falls back to Render URL
const API_BASE = (import.meta.env.VITE_API_URL || 'https://trustguard-phase2.onrender.com').replace(/\/$/, '');
...
async function _refreshSessionToken() {
  ...
  try {
    const idToken = await _firebaseUser.getIdToken(true);
...
const apiFetch = async (url, opts = {}, _isRetry = false) => {
  ...
  if (res.status === 401 && !_isRetry) {
    ...
    if (newToken) return apiFetch(url, opts, true);
  }
  return res;
};
...
function AuthScreen({ onAuth, dark, toggleDark }) {
  ...
  const submitAdmin = async (e) => {
    ...
    if (adminPw === ADMIN_USER.password) {
      ...
      try {
        const r = await fetch(`${API_BASE}/api/auth/login`, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ email:ADMIN_USER.email, password:ADMIN_USER.password }) });
      ...
      if (isFirebaseConfigured()) {
        ...
        if (mode === 'login') {
          ...
          try {
            ...
            const r = await fetch(`${API_BASE}/api/auth/firebase-verify`, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({idToken}) });
            const d = await r.json();
        ...
        if (mode === 'login') {
          if (form.email === ADMIN_USER.email && form.password === ADMIN_USER.password) {
            const u = { ...ADMIN_USER }; delete u.password;
            try {
              const r = await fetch(`${API_BASE}/api/auth/login`, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({email:form.email,password:form.password}) });
              const d = await r.json();
              if (d.token) { setSessionToken(d.token); u.sessionToken = d.token; }
            } catch(_) {}
            localStorage.setItem('tg_user', JSON.stringify(u)); onAuth(u);
          ...
          try {
            const r = await fetch(`${API_BASE}/api/auth/register`, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({email:form.email,password:form.password,name:form.name,orgKey:form.orgKey}) });
            const d = await r.json();
...
export default function App() {
  ...
  const loadHistory = async () => {
    ...
    setHistLoad(true);
    try {
      const res = await apiFetch(`${API_BASE}/api/chat/history?limit=60`);
      const data = await res.json();
  ...
  const loadSession = async (sid) => {
    ...
    try {
      const res = await apiFetch(`${API_BASE}/api/chat/session/${sid}`);
    ...
    const poll = async () => {
      try {
        const [sR, hR] = await Promise.all([
          apiFetch(`${API_BASE}/stats?t=${Date.now()}`),
          apiFetch(`${API_BASE}/health?t=${Date.now()}`),
        ]);
  ...
  const logout = async () => {
    ...
    try { await apiFetch(`${API_BASE}/api/auth/logout`, { method:'POST' }); } catch(_) {}
  ...
  const sendMsg = async () => {
    ...
    setTyping(true);
    try {
      const res  = await apiFetch(`${API_BASE}/api/chat`, { method:'POST', body:JSON.stringify({ message:txt, source:'Internal Chat', session_id:currentSid }) });
      const data = await res.json();
      setTyping(false);
      ...
      setTyping(false);
      setMsgs(p => [...p, { id:Date.now()+1, sender:'bot', text:'⚠ Could not reach backend.' }]);
    }
  };
  ...
  const doAnalyze = async (msgId, text, userQuery) => {
    ...
    try {
      const res  = await apiFetch(`${API_BASE}/analyze`, {
        method:'POST',
        body: JSON.stringify({ text, user_query: userQuery || text }),
      });
      const data = await res.json();
      setAnalysis(p => ({ ...p, [msgId]:{ loading:false, data } }));

      const correction = data.hallucination_info?.correction;
      const hasCorrection = data.hallucination_info?.detected && correction && !['N/A','None','n/a','none',''].includes(correction.trim());
      const hasRedaction = data.pii_info?.detected && data.pii_info?.refined_text && data.pii_info.refined_text !== text;

      if (hasCorrection) {
        setMsgs(prev => prev.map(m => m.id === msgId ? { ...m, text: correction.trim(), _corrected:true, _originalText: m._originalText || m.text } : m));
        addNotif('✓ Response corrected — hallucination replaced with live data', 'warn');
      } else if (hasRedaction) {
        setMsgs(prev => prev.map(m => m.id === msgId ? { ...m, text: data.pii_info.refined_text, _redacted:true, _originalText: m._originalText || m.text } : m));
...
function VerifyView({ verifyData, verifyLoading, onClose }) {
  ...
  if (!verifyData) return <div style={{ flex:1,display:'flex',alignItems:'center',justifyContent:'center',color:'var(--text3)' }}>No verification data</div>;
  if (verifyData.error) return <div style={{ flex:1,display:'flex',alignItems:'center',justifyContent:'center',color:'var(--danger)',fontSize:14 }}>Error: {verifyData.error}</div>;
  ...
  return (
    <div className="fade-up" style={{ padding:'28px 32px',maxWidth:820,margin:'0 auto',width:'100%' }}>
      <div style={{ display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:24 }}>
        <div>
          <h2 className="serif" style={{ fontSize:26,color:'var(--text)',letterSpacing:'-0.4px',marginBottom:6 }}>Verification Report</h2>
          <div style={{ display:'flex',alignItems:'center',gap:10,flexWrap:'wrap' }}>
            ...
            {sourceUrl && <a href={sourceUrl} target="_blank" rel="noopener noreferrer" style={{ fontSize:12,color:'var(--info)',textDecoration:'underline',textUnderlineOffset:2 }}>Open original page ↗</a>}
          </div>
        </div>
        <button onClick={onClose} className="btn-ghost" style={{ padding:'7px 14px',fontSize:13 }}><X size={13}/> Close</button>
      </div>

      <div style={{ padding:'18px 22px',borderRadius:14,marginBottom:18,background:allClear?'var(--ok-bg)':'var(--danger-bg)',border:`1.5px solid ${allClear?'rgba(30,169,124,.25)':'rgba(224,75,58,.25)'}`,display:'flex',alignItems:'center',gap:14 }}>
        <div style={{ width:44,height:44,borderRadius:12,flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center',background:allClear?'rgba(30,169,124,.15)':'rgba(224,75,58,.15)' }}>
          {allClear ? <CheckCircle size={24} color="var(--ok)"/> : <AlertTriangle size={24} color="var(--danger)"/>}
        </div>
        <div>
          <div style={{ fontSize:16,fontWeight:700,color:'var(--text)',marginBottom:3 }}>{allClear ? '✓ Content verified — no issues found' : 'Issues detected in this response'}</div>
          <div style={{ fontSize:13,color:'var(--text2)',lineHeight:1.5 }}>
            {[!hallOk && 'Hallucination detected', !piiOk && 'Private information (PII) found'].filter(Boolean).join(' · ') || 'Content verified against live sources.'}
          </div>
        </div>
      </div>
      ...
      <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:14 }}>
        <div className="card" style={{ padding:20 }}>
          <div style={{ fontSize:13,fontWeight:700,color:'var(--text)',marginBottom:12,display:'flex',alignItems:'center',gap:7 }}><FileText size={14} color="var(--text3)"/> Original Response</div>
          <div style={{ fontSize:13.5,color:'var(--text2)',lineHeight:1.7,maxHeight:180,overflowY:'auto',padding:'10px 14px',borderRadius:9,background:'var(--bg2)',border:'1px solid var(--border)' }}>{verifyData.original_text}</div>
        </div>
        <div className="card" style={{ padding:20 }}>
          <div style={{ fontSize:13,fontWeight:700,color:'var(--text)',marginBottom:12,display:'flex',alignItems:'center',gap:7 }}>
            <ShieldCheck size={14} color={allClear?'var(--ok)':'var(--accent)'}/> {allClear ? 'Verified Text (unchanged)' : 'Refined / Corrected Text'}
          </div>
          <div style={{ fontSize:13.5,color:'var(--text)',lineHeight:1.7,maxHeight:180,overflowY:'auto',padding:'10px 14px',borderRadius:9,background:allClear?'var(--ok-bg)':'var(--accent-bg)',border:`1px solid ${allClear?'rgba(30,169,124,.2)':'rgba(92,71,224,.2)'}` }}>
            {analysis.hallucination_info?.correction && !['N/A','None','n/a'].includes(analysis.hallucination_info.correction) ? analysis.hallucination_info.correction : analysis.pii_info?.refined_text || verifyData.original_text}
          </div>
        </div>
      </div>
    </div>
        ...
        <div style={{ padding:'12px 24px 20px', flexShrink:0, background:'var(--surface)' }}>
          {readOnly ? (
            ...
            <div className="chat-composer" style={{ maxWidth:860,margin:'0 auto' }}>
              <textarea ref={textareaRef} className="chat-textarea" placeholder="Ask anything — I'll verify and scan in real time…"
                value={input} onChange={e => { setInput(e.target.value); resizeTextarea(); }}
              ...
              <div className="fade-in" style={{ display:'flex',flexDirection:'column',gap:10 }}>
                <div className="card-sm" style={{ padding:13,background:d.hallucination_info?.detected?'var(--danger-bg)':'var(--ok-bg)',border:`1px solid ${d.hallucination_info?.detected?'rgba(224,75,58,.2)':'rgba(30,169,124,.2)'}` }}>
                  <div style={{ display:'flex',alignItems:'center',gap:7,marginBottom:8 }}>
                    ...
                    <span style={{ fontSize:11,fontWeight:700,color:d.hallucination_info?.detected?'var(--danger)':'var(--ok)' }}>{d.hallucination_info?.detected ? 'Flagged' : 'Verified ✓'}</span>
                  </div>
                  <p style={{ fontSize:11.5,color:'var(--text2)',lineHeight:1.6,margin:0 }}>
                    {d.hallucination_info?.reason && d.hallucination_info.reason !== 'N/A' ? d.hallucination_info.reason : 'Content verified against live sources.'}
                  </p>
                ...
                <div className="card-sm" style={{ padding:13,background:d.pii_info?.detected?'var(--danger-bg)':'var(--ok-bg)',border:`1px solid ${d.pii_info?.detected?'rgba(224,75,58,.2)':'rgba(30,169,124,.2)'}` }}>
                  <div style={{ display:'flex',alignItems:'center',gap:7,marginBottom:8 }}>
                    ...
                    <span style={{ fontSize:11,fontWeight:700,color:d.pii_info?.detected?'var(--danger)':'var(--ok)' }}>{d.pii_info?.detected ? 'Detected' : 'Clean ✓'}</span>
                  </div>
        ...
        <div className="card" style={{ padding:18,overflow:'hidden' }}>
          ...
          {riskLogs.length === 0 ? (
            <div style={{ display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',height:80,color:'var(--text3)',fontSize:12,gap:7 }}>
              <CheckCircle size={22} color="var(--ok)"/><span>No threats detected</span>
            </div>
          ) : riskLogs.map((log,i) => (
      ...
      <div className="card" style={{ overflow:'hidden' }}>
        <div style={{ padding:'14px 18px',borderBottom:'1px solid var(--border)',display:'flex',justifyContent:'space-between',alignItems:'center' }}>
          ...
          <div style={{ display:'flex',alignItems:'center',gap:8 }}>
            ...
            <div style={{ display:'flex',alignItems:'center',gap:5 }}>
              <span className="status-dot live"/><span style={{ fontSize:11,fontWeight:600,color:'var(--ok)' }}>Live</span>
            </div>
          </div>
        </div>
        ...
        {[
          ...
          <div key={i} className="card" style={{ padding:18,display:'flex',alignItems:'center',gap:14 }}>
            <div style={{ position:'relative',width:52,height:52,flexShrink:0 }}>
              ...
              <div style={{ position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center' }}><m.icon size={12} color={m.color}/></div>
            </div>
            <div>
...
function ActivePreviewCard({ prefs, prefStatus }) {
  ...
  const testPersonalization = async () => {
    ...
    try {
      const res  = await apiFetch(`${API_BASE}/api/personalization/debug`);
      const data = await res.json();
      setDebugData(data);
    } catch(e) {
    ...
    <div className="fade-in" style={{ marginBottom:14 }}>
      <div style={{ padding:'14px 18px', borderRadius:11, background:'var(--accent-bg)', border:'1px solid rgba(92,71,224,.2)' }}>
        ...
        <div style={{ display:'flex', flexWrap:'wrap', gap:5 }}>
          {active.map((tag,i) => (
            <span key={i} style={{ fontSize:11.5, padding:'3px 9px', borderRadius:12, background:'var(--surface2)', border:'1px solid rgba(92,71,224,.15)', color:'var(--text2)', fontWeight:500 }}>{tag}</span>
          ))}
        </div>
        {open && debugData && (
            ...
            {debugData.error ? (
              ...
              <>
                ...
                <details style={{ cursor:'pointer' }}>
                  <summary style={{ fontSize:11.5, color:'var(--text3)', fontWeight:600, userSelect:'none' }}>View system prompt ({debugData.prompt_len} chars)</summary>
...
function MemoryView({ sessionToken }) {
  ...
  const load = useCallback(async () => {
    try {
      const res  = await localFetch(`${API_BASE}/api/memory`);
      const data = await res.json();
      if (res.ok) {
  ...
  const addMemory = async () => {
    ...
    try {
      const res  = await localFetch(`${API_BASE}/api/memory`, { method:'POST', body:JSON.stringify({ text }) });
      const data = await res.json();
      if (res.ok) { setNewText(''); await load(); } else { setError(data.error || 'Could not save memory'); }
  ...
  const deleteMemory = async (id) => {
    ...
    try {
      await localFetch(`${API_BASE}/api/memory/${id}`, { method:'DELETE' });
  ...
  const clearAll = async () => {
    setClearing(true);
    try {
      await localFetch(`${API_BASE}/api/memory/clear`, { method:'POST' });
  ...
  return (
    <div>
      <div className="card" style={{ padding:20, marginBottom:14 }}>
        <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:16, gap:12 }}>
          <div>
            <h3 style={{ fontSize:15,fontWeight:700,color:'var(--text)',display:'flex',alignItems:'center',gap:9,marginBottom:5 }}>
              <div style={{ width:28,height:28,borderRadius:8,background:'var(--accent-bg)',display:'flex',alignItems:'center',justifyContent:'center' }}><Brain size={14} color="var(--accent)"/></div>
              Manage Memory
            </h3>
            <p style={{ fontSize:12.5,color:'var(--text3)',lineHeight:1.6,maxWidth:420 }}>
              TrustGuard AI automatically saves key facts from your conversations and uses them to give more relevant, personalised responses.
            </p>
          </div>
          {count > 0 && (
            <button onClick={()=>setShowClearConfirm(true)} className="btn btn-ghost" style={{ fontSize:12,padding:'6px 12px',flexShrink:0,color:'var(--danger)',borderColor:'rgba(224,75,58,.25)' }}>
        ...
        <div className="memory-bar"><div className="memory-bar-fill" style={{ width:`${pctUsed}%`, background:barColor }}/></div>
        {full && <p className="fade-in" style={{ fontSize:12,color:'var(--danger)',marginTop:8,display:'flex',alignItems:'center',gap:5 }}><AlertCircle size={12}/> Memory full — delete some to allow new memories.</p>}
      </div>
      ...
      {showClearConfirm && (
        <div className="fade-in" style={{ padding:'16px 18px',borderRadius:12,background:'var(--danger-bg)',border:'1.5px solid rgba(224,75,58,.25)',marginBottom:14 }}>
          <p style={{ fontSize:13.5,fontWeight:600,color:'var(--text)',marginBottom:12 }}>Delete all {count} memories? This cannot be undone.</p>
          <div style={{ display:'flex',gap:8 }}>
            <button onClick={clearAll} disabled={clearing} className="btn btn-danger" style={{ fontSize:12.5 }}>
      ...
      <div className="card" style={{ padding:18, marginBottom:14 }}>
        <label style={{ fontSize:12,fontWeight:700,color:'var(--text2)',textTransform:'uppercase',letterSpacing:'.4px',display:'block',marginBottom:10 }}>Add a memory manually</label>
        <div className="mem-add-row">
          <input className="mem-add-input" value={newText} placeholder='e.g. "The user prefers Python over JavaScript"'
            ...
            disabled={full}/>
          <button onClick={addMemory} disabled={!newText.trim()||adding||full} className="btn btn-primary" style={{ fontSize:13,padding:'9px 16px',flexShrink:0 }}>
      ...
      <div className="card" style={{ padding:18 }}>
        <div style={{ fontSize:13,fontWeight:700,color:'var(--text)',marginBottom:14,display:'flex',alignItems:'center',justifyContent:'space-between' }}>
          <span style={{ display:'flex',alignItems:'center',gap:6 }}><Brain size={13} color="var(--accent)"/> Saved memories</span>
        ...
        {loading ? (
          ...
          <div style={{ textAlign:'center',padding:'28px 0',color:'var(--text3)' }}>
            ...
            <p style={{ fontSize:12.5 }}>Start chatting and TrustGuard AI will automatically remember things about you.</p>
          </div>
        ) : (
          <div style={{ display:'flex',flexDirection:'column',gap:7 }}>
            {[...memories].reverse().map((mem, i) => (
              <div key={mem.id} className="memory-card fade-in" style={{ animationDelay:`${i*0.04}s` }}>
                ...
                <button className="mem-del" onClick={()=>deleteMemory(mem.id)} disabled={deleting.has(mem.id)} title="Delete this memory">
                  {deleting.has(mem.id) ? <div className="spin" style={{ width:11,height:11,border:'1.5px solid rgba(224,75,58,.3)',borderTopColor:'var(--danger)',borderRadius:'50%' }}/> : <X size={13}/>}
                </button>
              </div>
            ))}
          </div>
...
function SettingsView({ user, dark, toggleDark, sessionToken }) {
  ...
  useEffect(() => {
    localFetch(`${API_BASE}/api/settings`)
  ...
  useEffect(() => {
    const tok = sessionToken || getSessionToken();
    if (!tok) { setPrefLoaded(true); return; }
    localFetch(`${API_BASE}/api/personalization`)
      .then(r => { if (!r.ok) return { prefs: {} }; return r.json(); })
      .then(d => { if (d.prefs && Object.keys(d.prefs).length > 0) setPrefs(p => ({ ...DEFAULT_PREFS, ...d.prefs })); setPrefLoaded(true); })
  ...
  useEffect(() => {
    ...
    cfgTimer.current = setTimeout(() => {
      setCfgStatus('saving');
      localFetch(`${API_BASE}/api/settings`, { method:'POST', body:JSON.stringify(cfg) })
  ...
  useEffect(() => {
    ...
    prefTimer.current = setTimeout(async () => {
      ...
      try {
        const res = await localFetch(`${API_BASE}/api/personalization`, { method: 'POST', body: JSON.stringify(prefs) });
        if (!res.ok) { setPrefStatus('error'); setTimeout(() => setPrefStatus('idle'), 3500); return; }
        setPrefStatus('saved'); setTimeout(() => setPrefStatus('idle'), 2500);
      } catch (e) { setPrefStatus('error'); setTimeout(() => setPrefStatus('idle'), 3000); }
    }, 800);
  ...
  const PillSelect = ({ value, onChange, options }) => (
    <div className="pill-select">
      {options.map(opt => (
        <button key={opt.value} className={`pill-option ${value===opt.value?'selected':''}`} onClick={()=>onChange(opt.value)}>{opt.label}</button>
      ))}
    </div>
  );
    ...
    <div className="fade-up" style={{ padding:'28px 32px', maxWidth:780, margin:'0 auto' }}>
      ...
      <div className="card" style={{ padding:20,marginBottom:18,display:'flex',alignItems:'center',gap:16 }}>
        ...
        <div style={{ flex:1 }}>
          ...
          <div style={{ fontSize:12,color:'var(--text3)',marginTop:2 }}>{prefs.occupation ? prefs.occupation : user.email||'No email'}</div>
        ...
        {TABS.map(({id,label,icon:Icon})=>(
          <button key={id} onClick={()=>setActiveTab(id)}
            style={{ flex:1,padding:'9px 0',borderRadius:9,border:'none',fontSize:13,fontWeight:600,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:6,fontFamily:'Geist,sans-serif',transition:'all .15s',
              background:activeTab===id?'var(--surface)':'transparent', color:activeTab===id?'var(--text)':'var(--text3)', boxShadow:activeTab===id?'var(--sh)':'none' }}>
            <Icon size={13}/>{label}
          </button>
        ))}
      </div>
      ...
      {activeTab==='personalization' && (
        <div className="fade-in">
            ...
            <div style={{ display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:22,gap:16 }}>
              <div style={{ flex:1 }}>
                <h3 style={{ fontSize:15,fontWeight:700,color:'var(--text)',display:'flex',alignItems:'center',gap:9,marginBottom:6 }}>
                  <div style={{ width:28,height:28,borderRadius:8,background:'var(--accent-bg)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0 }}><User size={14} color="var(--accent)"/></div>
                  About you
                </h3>
                <p style={{ fontSize:12.5,color:'var(--text3)',lineHeight:1.6,maxWidth:380 }}>Help TrustGuard AI understand who you are.</p>
              </div>
            ...
            {!prefLoaded ? (
              ...
              <>
                <div className="pref-section">
                  <label className="pref-label"><span style={{width:6,height:6,borderRadius:'50%',background:'var(--accent)',display:'inline-block'}}/> What should TrustGuard AI call you?</label>
                  <input className="pref-field" placeholder="e.g. Alex, Dr. Kumar…" value={prefs.nickname} onChange={e=>setPrefs(p=>({...p,nickname:e.target.value}))}/>
                </div>
                <div className="pref-section">
                  <label className="pref-label"><span style={{width:6,height:6,borderRadius:'50%',background:'var(--accent)',display:'inline-block'}}/> What do you do?</label>
                  <input className="pref-field" placeholder="e.g. Security analyst, Developer, Student…" value={prefs.occupation} onChange={e=>setPrefs(p=>({...p,occupation:e.target.value}))}/>
                </div>
                <div className="pref-section">
                  <label className="pref-label"><span style={{width:6,height:6,borderRadius:'50%',background:'var(--accent)',display:'inline-block'}}/> Anything else the AI should know?</label>
                  <textarea className="pref-field" placeholder="Interests, working style, preferences…" value={prefs.about} onChange={e=>setPrefs(p=>({...p,about:e.target.value}))}/>
                </div>
              </>
            )}
          </div>
          ...
          <div className="card" style={{ padding:22,marginBottom:14 }}>
            <h3 style={{ fontSize:14,fontWeight:700,color:'var(--text)',marginBottom:4,display:'flex',alignItems:'center',gap:7 }}><FileText size={15} color="var(--accent)"/> Base style and tone</h3>
            <p style={{ fontSize:12,color:'var(--text3)',marginBottom:18 }}>Set how TrustGuard AI communicates.</p>
            <div className="pref-section">
          ...
          <div className="card" style={{ padding:22,marginBottom:14 }}>
            <h3 style={{ fontSize:14,fontWeight:700,color:'var(--text)',marginBottom:4,display:'flex',alignItems:'center',gap:7 }}><Sparkles size={15} color="var(--accent)"/> Characteristics</h3>
            <p style={{ fontSize:12,color:'var(--text3)',marginBottom:16 }}>Additional traits on top of your base style</p>
            {[
              { id:'warm',         emoji:'🤍', label:'Warm',            desc:'Empathetic and encouraging' },
              { id:'enthusiastic', emoji:'⚡', label:'Enthusiastic',    desc:'Energetic and upbeat' },
              { id:'headersLists', emoji:'📋', label:'Headers & Lists', desc:'Well-structured answers with sections and bullets' },
              { id:'emoji',        emoji:'😊', label:'Emoji',           desc:'Light use of relevant emoji' },
            ].map(({id,emoji,label,desc}) => (
              <div key={id} className="char-toggle">
                <div style={{ display:'flex',alignItems:'center',gap:13,flex:1 }}>
                  <div style={{ width:38,height:38,borderRadius:10,background:prefs[id]?'var(--accent-bg)':'var(--bg3)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,transition:'all .2s',fontSize:17,border:prefs[id]?'1.5px solid rgba(92,71,224,.25)':'1.5px solid transparent' }}>{emoji}</div>
                  <div style={{ flex:1 }}>
          ...
          <div className="card" style={{ padding:22,marginBottom:14 }}>
            <h3 style={{ fontSize:14,fontWeight:700,color:'var(--text)',marginBottom:4,display:'flex',alignItems:'center',gap:7 }}><FileText size={15} color="var(--accent)"/> Custom instructions</h3>
            <p style={{ fontSize:12,color:'var(--text3)',marginBottom:14,lineHeight:1.6 }}>Write anything you want the AI to always follow.</p>
            <textarea className="pref-field" style={{ minHeight:130 }}
              placeholder="e.g. Always start with a 1-line summary · Reply in same language · Use real-world examples"
      ...
      {activeTab==='memory' && <div className="fade-in"><MemoryView sessionToken={sessionToken}/></div>}
